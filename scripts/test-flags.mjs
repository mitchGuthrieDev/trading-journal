/* A14 — guard the client↔Worker feature-flag default drift. The app's APP_FLAGS (src/app/lib/flags.ts)
   and the Worker's DEFAULTS.flags (functions/api/config.ts) are duplicated literals coupled only by a
...
const def = flagsFrom(
  ...
  /DEFAULTS(?::\s*\w+)?\s*=\s*\{\s*flags:\s*\{([^}]*)\}/,
  'functions/api/config.ts'
);

console.log('A14 — feature-flag defaults: src/app/lib/flags.ts APP_FLAGS vs functions/api/config.ts DEFAULTS.flags');

...
import { $, on, pad2, fmtDate, PAGE_MODE, STAGING_PAGE, emit, compute, DOW_LABEL, BROKERS, BROKER_ORDER, BROKER_FEEDS, STATES, DEMO_BROKER, DEMO_FEED, DEMO_STATE, usd, cls } from './core.js';
import { FILTERS, renderCalendar, renderDash, renderCurve, renderLoaded, resetApp, setDashVisible, activeMetrics, baseTrades, filtersActive, loadTradeMeta } from './render.js';
import { tradeCells, renderDataManager, annTextField, annTagsField, annShotsField, annCapture, annAddShot, reloadFromStore } from './datamanager.js';
...
export function flag(k){ return !!APP_FLAGS[k]; }
...
export const stageEls=ctx=> ctx==='manage'
...
export function reparseStage(ctx,isAuto){
  if(!state.PENDING || state.PENDING.ctx!==ctx){ return; }
  const {sel}=stageEls(ctx);
  const override = sel ? sel.value : '';
  let r;
  try{ r=Adapters.parse(state.PENDING.rawText, override||undefined); }
...
export function onPlatformChange(ctx){ reparseStage(ctx,/*isAuto*/false); }
...
export async function commitPending(ctx){
  ...
  const trades=state.PENDING.result.trades, name=state.PENDING.name;
...
export function onFiltersChanged(){
  if(!state.TRADES.length){ updateFilterCount(); return; }
...
export function allTags(){
  const s=new Set(); for(const m of state.TRADE_META.values()) (m.tags||[]).forEach(t=>s.add(t));
  return [...s].sort();
}
...
export function initFilters(){
  ...
  on('f_saved','change',e=>{ const s=state.SAVED_FILTERS.find(x=>x.id===e.target.value); if(s) applyFilterObj(s.f); });
}

/* ---- saved filters (name, recall, manage) — shared UI, all surfaces (CH16) ---- */
export async function loadSavedFilters(){
  if(!Store.available()){ state.SAVED_FILTERS=[]; return; }
  try{ state.SAVED_FILTERS=(await Store.getMeta('savedFilters'))||[]; }catch(_){ state.SAVED_FILTERS=[]; }
  syncSavedFilterSelect();
}
export async function persistSavedFilters(){ if(Store.available()){ try{ await Store.setMeta('savedFilters', state.SAVED_FILTERS); }catch(_){} } }
export function currentFilterObj(){ return { from:FILTERS.from, to:FILTERS.to, symbol:FILTERS.symbol, side:FILTERS.side, session:FILTERS.session, tag:FILTERS.tag, dows:[...FILTERS.dows] }; }
export function syncSavedFilterSelect(){
  const sel=$('f_saved'); if(!sel) return; const cur=sel.value;
  sel.innerHTML='<option value="">— Saved filters —</option>'+state.SAVED_FILTERS.map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join('');
...
export async function deleteSavedFilter(id){ state.SAVED_FILTERS=state.SAVED_FILTERS.filter(s=>s.id!==id); await persistSavedFilters(); syncSavedFilterSelect(); }
export async function renameSavedFilter(id){ const s=state.SAVED_FILTERS.find(x=>x.id===id); if(!s) return;
...
export async function selectDay(d){
  ...
  if(state.selectedDate){ const cell=document.querySelector(`#cal .cell[data-date="${state.selectedDate}"]`); if(cell) cell.classList.add('selday'); }
...
export function jumpToLatest(){
  ...
  state.calYear=yy; state.calMonth=mm-1;
  renderCalendar();
...
export function selectFromGraph(d){
  ...
  const [yy,mm]=d.split('-').map(Number); state.calYear=yy; state.calMonth=mm-1;
  renderCalendar();
...
export async function persistDay(snap){
  ...
  if(snap.date===state.selectedDate){ const st=document.getElementById('j_stat'); if(st) st.textContent = nonEmpty?'saved':''; }
...
export function flushDayNow(){ clearTimeout(jSaveTimer); const s=daySnapshot(); if(s) persistDay(s); }
export function scheduleDaySave(){ clearTimeout(jSaveTimer); const s=daySnapshot(); jSaveTimer=setTimeout(()=>persistDay(s),500); }
...
export function cancelDaySave(){ clearTimeout(jSaveTimer); jSaveTimer=null; }
...
export async function updateJournalEditor(){
  ...
  if(!state.selectedDate){ DAY_EDIT=null; if(body) body.innerHTML=''; renderDayTrades(null); if(stat) stat.textContent=''; return; }
  ...
  if(state.DEMO_MODE || !Store.available()){
    ...
    if(hint){ hint.style.display=''; hint.textContent = state.DEMO_MODE ? 'Day-notes are disabled for the demo dataset.'
      : 'Local storage is unavailable in this browser, so notes can’t be saved.'; }
    return;
  }
  if(hint) hint.style.display='none'; if(label) label.textContent='Notes — '+state.selectedDate;
...
export function armStagingLanding(metaHtml){
  state.STAGING_DATA_READY=true; STAGING_META=metaHtml||'';
...
export function gateOk(){ return !!($('c_broker').value && $('c_feed').value && $('c_state_sel').value); }
...
export function recalc(){ if(state.METRICS_ALL) renderDash(); }

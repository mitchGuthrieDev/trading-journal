<script lang="ts">
  ...
  // A97 (R18, staging-first): the per-metric definitions now live in the panels/modals that own each
  // number (AdvancedStats "Assumptions", the F14 stat-card modals, CostPanel, EquityCurve, CalendarMonth,
  // the scope toggle). On STAGING this panel is therefore trimmed to just the cross-cutting *parsing*
  // caveats that gate every grouping and need one discoverable home (what a "trade" is; US-date/Eastern-
  // time assumptions). Prod/demo keep the full standalone glossary until the distribution is promoted (CH16).
